...
const Servers: NextPage<ServersProps> = ({ data, desc, date, user, theme }) => {
	...
	useEffect(() => {
		if (data)
			Fetch<User[]>(`/servers/${data.id}/owners`).then(async (res) => {
			...
			<Container paddingTop className='py-10'>
				<NextSeo
					...
					openGraph={{
						images: [
							{
								url: KoreanbotsEndPoints.OG.server(data.id, data.name, data.intro, data.category, [
							...
							{data.state === 'unreachable' ? (
								<Message type='error'>
									...
									{owners?.find((el) => el.id === user?.id) && (
										<>
											<h3 className='text-md pt-2 font-bold'>서버 관리자시군요!</h3>
											<p>
												봇을 서버에서 추방하셨다면{' '}
												<a
													className='cursor-pointer text-blue-600 hover:text-blue-500'
													href={`${DiscordEnpoints.InviteApplication(
														DSKR_BOT_ID,
														{},
														'bot',
														null,
						...
						<div className='w-full lg:flex'>
							...
							<div className='w-full grow px-5 py-12 text-center lg:w-5/12 lg:text-left'>
								<h1 className='mb-2 mt-3 text-4xl font-bold' style={bg ? { color: 'white' } : {}}>
									...
									{checkServerFlag(data.flags, 'trusted') ? (
										<Tooltip
											placement='bottom'
											overlay='해당 서버는 한국 디스코드 리스트에서 엄격한 기준을 통과한 서버 입니다!'
										>
							...
							<div className='w-full lg:w-1/4'>
								{['ok', 'unreachable'].includes(data.state) && (
									<LongButton newTab href={`/servers/${router.query.id}/join`}>
							...
							<div className='mb-1 w-full lg:w-1/4'>
								...
								{data.emojis.length !== 0 && (
									<>
										...
										<div className='flex flex-wrap'>
											{data.emojis.slice(0, 5).map((el) => (
												<Image src={el.url} key={el.name} className='m-1 h-8' />
											))}
											{data.emojis.length > 5 && (
												<Tag
													className='cursor-pointer'
													onClick={() => setEmojisModal(true)}
													text={`+${data.emojis.length - 5}개`}
												/>
											)}
										</div>
										<Modal
										...
										>
											...
											<div className='flex flex-wrap'>
												{data.emojis.map((el) => (
													...
													>
														<div>
															<Image src={el.url} className='m-1 h-8' />
														</div>
													</Tooltip>
												))}
											</div>
								...
								<Modal
									...
									dark={theme === 'dark'}
								>
									<div className='grid grid-rows-1 gap-x-1 md:grid-cols-2'>
										{owners ? (
											owners.map((el) => (
												<Owner
													key={el.id}
