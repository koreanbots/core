...
const ManageServerPage:NextPage<ManageServerProps> = ({ server, user, owners, csrfToken, theme }) => {
	...
	return <Container paddingTop className='pt-5 pb-10'>
		...
		<Formik initialValues={cleanObject({
			...
			webhook_status: server.webhook_status,
			_csrf: csrfToken
		})}
		...
		onSubmit={submitServer}>
			{({ errors, touched, values, setFieldTouched, setFieldValue }) => (
				<Form>
					...
					<Label For='webhook' label='웹훅 링크' labelDesc='봇의 업데이트 알림을 받을 웹훅 링크를 입력해주세요. (웹훅 링크가 유효하지 않을 경우 웹훅이 중지되며, 다시 저장할 경우 작동합니다.)' error={errors.webhook && touched.webhook ? errors.webhook : null} warning={server.webhook_status === WebhookStatus.Disabled} warningText='웹훅 링크가 유효하지 않아 웹훅이 중지되었습니다.'>
						<Input name='webhook' placeholder='https://discord.com/api/webhooks/ID/TOKEN' warning={server.webhook_status === WebhookStatus.Disabled}/>
					</Label>
					<Divider />
