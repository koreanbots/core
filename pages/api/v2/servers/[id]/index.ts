...
const Servers = RequestHandler()
	...
	.delete(async (req: DeleteApiRequest, res) => {
		...
		if((!data || server.state === 'unreachable') && (await DiscordBot.fetchInvite(server.invite).catch(() => false))) return ResponseWrapper(res, { code: 400, message: '해당 서버의 정보를 불러올 수 없습니다.', errors: ['봇이 추방되었거나, 오프라인이 아닌지 확인하시고 다시 시도해주세요.'] })
